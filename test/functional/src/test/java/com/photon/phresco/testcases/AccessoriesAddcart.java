...
public class AccessoriesAddcart extends TestCase {
	...
	@Test
	public void testAccessories() throws InterruptedException, IOException,
			Exception {
		...
		try {
			...
			assertNotNull("selenium-port number should not be null",
					SELENIUM_PORT);
			wel = new WelcomeScreen(phr.SERVER_HOST,
					SELENIUM_PORT, browserAppends, serverURL, phr.SPEED,
					phr.CONTEXT);
			assertNotNull(wel);
